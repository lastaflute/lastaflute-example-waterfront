 ...
 * @author FreeGen
 */
public interface WaterfrontEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/waterfront */
    ...
    /** The key of the configuration. e.g. waterfront-support@annie.example.com */
     ...
     * The value is, e.g. /tmp/lastaflute/waterfront <br>
     ...
     * The value is, e.g. waterfront-support@annie.example.com <br>
     * comment: Mail Address for Waterfront Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements WaterfrontEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(WaterfrontEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(WaterfrontEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(WaterfrontEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(WaterfrontEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(WaterfrontEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(WaterfrontEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(WaterfrontEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(WaterfrontEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(WaterfrontEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(WaterfrontEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(WaterfrontEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(WaterfrontEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(WaterfrontEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(WaterfrontEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(WaterfrontEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(WaterfrontEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(WaterfrontEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(WaterfrontEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(WaterfrontEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(WaterfrontEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(WaterfrontEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(WaterfrontEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(WaterfrontEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(WaterfrontEnv.SERVER_DOMAIN);
        }
    }
}
